### REPEATED COMMANDS ###
execute as @a[gamemode=!spectator,tag=PO] run function pc_core:particles
execute as @p[nbt={Sleeping:1b}] if score IsSleepingBool Database matches 0 run tag @s add DoSleepVote
execute as @p[tag=DoSleepVote] if score IsSleepingBool Database matches 0 run scoreboard players set IsSleepingBool 1
execute as @p[tag=DoSleepVote] at @s run function pc_core:vs_m
### FUNCTIONS ###
# DONATOR PERKS
execute as @a[scores={Secret=1..},tag=special] run function pc_core:zelda
# TPA
execute as @a[scores={TPA=1..}] run function pc_core:tpa
execute as @a[scores={TPA=..-1}] run function pc_core:tpa
# CREEPERS
execute as @e[type=creeper,tag=!creeped] run data merge entity @s {Tags:["creeped"],ExplosionRadius:0b}
# CHEATERS
execute as @a[scores={Diamond=1..}] run function pc_core:xray_check
execute as @a[scores={Netherite=1..}] run function pc_core:xray_check
# SPAWN //205 96 128
execute as @a[x=410,y=200,z=256,distance=500..700,tag=!spawn.bypass,tag=!op] at @s run function pc_core:bounce
execute as @a[x=410,y=200,z=256,distance=..500] at @s run effect give @s resistance 1 100 true
# 1/2 SLEEP
#execute as @a store result score @s sleep run data get entity @s SleepTimer
#execute if entity @p[scores={sleep=1..}] run function pc_core:sleep/half_sleep
### NO CREEPER EXPLODE ###
#execute as @e[type=creeper,nbt=!{ExplosionRadius:0}] run data merge entity @s {ExplosionRadius:0}
### ITEM CLEAR ###
execute if score IB Database >= 1 Database run function pc_core:item_cleanup
### KILLER ###
kill @a[tag=die]
#kill @e[type=tnt]
kill @e[type=tnt_minecart]
### SEASONAL ###
#function seasons:october/zombies/main
